...
- `docs/plan/M0/`：需求澄清与技术底座定稿（✅ 已完成）
- `docs/plan/M1/`：MVP 核心（构建期拉取 + 列表/详情渲染）（🚧 实现中）
...
P1（影响工程方案与路由）4. 站点技术栈：Next.js 静态导出（`output: "export"`）或 Vite SSG 等方案，团队偏好与部署环境约束是什么？5. 路由与兼容：是否需要子路径部署（例如 `/blog/`）？静态资源与路由的 basePath 需求是什么？

P2（不阻塞 MVP，但影响体验与后续工作量）6. SEO 预期：是否在 MVP 阶段就要求 `sitemap.xml` / RSS 输出？若不要求，计划进入哪个里程碑？
